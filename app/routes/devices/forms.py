"""PC est magique - Devices-related Forms"""
...
class DeviceRegistrationForm(FlaskForm):
    ...
    nom = wtforms.StringField(_l("Nom (optionnel)"), validators=[Optional(), Length(max=64)])
    type = wtforms.StringField(_l("Type (optionnel)"), validators=[Optional(), Length(max=64)])
    mac = wtforms.StringField(_l("Adresse MAC*"), validators=[DataRequired(), MacAddress()])
