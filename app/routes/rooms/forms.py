...
class RentalRegistrationForm(FlaskForm):
    ...
    start = html5.DateField(_l("Début de la location"), validators=[DataRequired(), PastDate()])
    end = html5.DateField(_l("Fin de la location (optionnel)"), validators=[Optional(), FutureDate()])
...
class RentalTransferForm(FlaskForm):
    ...
    end = html5.DateField(_l("Fin de la location"), validators=[DataRequired(), PastDate()])
