    ...
    while i<len(voeux):
        ...
        for voeu in voeux:
            if voeu.places_attribuees == 0:
                if voeu.spectacle.nb_tickets-sum_places_attribuees_spect(voeu.spectacle, voeux)-voeu.places_demandees >= 0:
                    voeu.places_attribuees = voeu.places_demandees
                    club_q_algo_logger.info(f"Full - {voeu.places_attribuees} places - {voeu.pceen.full_name} - {voeu.spectacle.nom} - Priority {voeu.priorite} - Discontent {voeu.pceen.discontent :.2f}")
                elif voeu.spectacle.nb_tickets-sum_places_attribuees_spect(voeu.spectacle, voeux)-places_minimum_handle(voeu.places_minimum) >= 0:
                    voeu.places_attribuees = voeu.spectacle.nb_tickets-sum_places_attribuees_spect(voeu.spectacle, voeux)
                    club_q_algo_logger.info(f"Partial - {voeu.places_attribuees} places - {voeu.pceen.full_name} - {voeu.spectacle.nom} - Priority {voeu.priorite} - Discontent {voeu.pceen.discontent :.2f} - {voeu.places_demandees} asked.")
                else:
                    club_q_algo_logger.info(f"Refused - {voeu.pceen.full_name} - {voeu.spectacle.nom} - Priority {voeu.priorite} - {sum_places_attribuees_spect(voeu.spectacle, voeux)} left - {voeu.places_demandees} asked")
                    if voeu.priorite<5:
                        for pceen in pceens:
                            if voeu.pceen.id == pceen.id:
                                pceen.discontent = pceen.discontent -2/3*voeu.priorite+8/3
                                club_q_algo_logger.info(f"Added {-2/3*voeu.priorite+8/3 :.2f} discontent. New discontent : {pceen.discontent :.2f}")
                    a = 0
                    for voeu_c in voeux:
                        if voeu._pceen_id == voeu_c._pceen_id and voeu_c != voeu and voeu_c.priorite > voeu.priorite:
                            voeu_c.priorite = voeu_c.priorite-1
                            club_q_algo_logger.info(f"Upgraded priority of Voeu {voeu_c.id} - {voeu_c.spectacle.nom} - Priority {voeu_c.priorite:} - Old priority : {voeu_c.priorite+1}")
                            a=1
                    if a == 0:
                        club_q_algo_logger.info(f"Did not find any more voeu for {voeu.pceen.full_name} after priority {voeu.priorite}")
                    break
            i+=1
        if i == 0:
            voeux = voeux[1:]
...
def sum_up(voeux, pceens):
    ...
    for voeu in voeux:
        if voeu.spectacle.id not in spectacles:
            club_q_algo_logger.info(f"{voeu.spectacle.nom} - {voeu.spectacle.nb_tickets} places - {sum_places_attribuees_spect(voeu.spectacle, voeux)} attribuées")
