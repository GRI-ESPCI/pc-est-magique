"""PC est magique Flask App - Devices Control Models"""
...
class Device(Model):
    ...
    def __repr__(self) -> str:
        """Returns repr(self)."""
        return f"<Device #{self.id} ('{self.name}') of {self.pceen}>"
    ...
    def allocate_ip_for(self, room: models.Room) -> str:
        ...
        Returns:
            The allocated IP.
        """
        if self.pceen.is_banned:
            # PCeen banned: IP in 10.0.8-255.0-255 (encoding ban ID)
            # (i.e. a range of 126975 bans)
            ban = self.pceen.current_ban
