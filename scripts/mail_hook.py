"""PC est magique - Transfert des mails reçus vers Discord
...
Ce script peut uniquement être appelé depuis Flask :
  ...
  * Soit par ligne de commande :
    cd /home/pc-est-magique/pc-est-magique; ./env/bin/flask script mail_hook.py
...
def report_mail() -> None:
    ...
    # Get matching PCeen and complete Embed
    sender_name, sender_address = email.utils.parseaddr(sender)
    if "@" in sender_address:
        pceen = PCeen.query.filter_by(email=sender_address).first()
        if pceen:
            embed.add_embed_field(
                name="PCeen correspondant :",
                value=pceen.full_name,
            )
            embed.add_embed_field(
                name="Profil (compte GRI nécessaire) :",
                value=flask.url_for("profile.main", doas=pceen.id),
            )
